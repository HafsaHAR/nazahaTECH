...
export default function CreateChallengeModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen) {
      console.log('[CreateChallengeModal] Modal est maintenant OUVERTE');
    }
  }, [isOpen]);
  ...
  const handleRemoveExtraField = (index, e) => {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  // Création d'un nœud dédié dans document.body pour éviter les collisions d'évènements DOM
  let portalContainer = document.getElementById('challenge-modal-portal');
  if (!portalContainer) {
    portalContainer = document.createElement('div');
    portalContainer.id = 'challenge-modal-portal';
    ...
    >
      ...
      >
        <div className="modal-header">
          <h2 className="modal-title">🏆 Créer un Nouveau Défi INPPLC</h2>
          <button
            type="button"
            onClick={(e) => {
              e.preventDefault();
          ...
          <div className="extra-fields-section">
            ...
            {extraFields.map((field, idx) => (
              <div key={idx} className="extra-field-card">
                <button
                  type="button"
                  onClick={(e) => handleRemoveExtraField(idx, e)}
